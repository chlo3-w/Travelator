...
class Comment {
    ...
    public static function readComments($postId) {
        $db = Db::getInstance();
        $stmt = $db->prepare('SELECT comments.ID, comments.postID, comments.userID, comments.commentBody, comments.parentID, users.avatar, users.first_name, users.last_name, createdAt
                            FROM comments
                            INNER JOIN users 
